...
## Tasks / Subtasks
- [ ] **Task 1: Create Claude Code Sub-Agent Framework (AC: 1, 2)**
  - [ ] Design sub-agent architecture that integrates with Claude Code's routing system
  - [ ] Implement sub-agent activation and initialization based on file context triggers
  - [ ] Create package loading mechanism that connects to Story 2.1 runtime components
  - [ ] Implement error handling for sub-agent initialization failures
  - [ ] Security validation: Ensure sub-agent only loads packages from secure project paths

- [ ] **Task 2: Implement Real-Time Security Guidance Generation (AC: 3, 4)**
  - [ ] Create context-aware guidance generation using AgenticRuntime from Story 2.1
  - [ ] Implement rule selection based on current file type and code context
  - [ ] Design non-intrusive display mechanism for security guidance
  - [ ] Create structured guidance format with actionable recommendations
  - [ ] Security validation: Sanitize user code input before LLM processing

- [ ] **Task 3: Develop Secure Code Snippet Suggestions (AC: 5)**
  - [ ] Implement secure code snippet generation from Rule Card examples
  - [ ] Create context-aware snippet recommendations based on detected security issues
  - [ ] Ensure snippets maintain code style consistency with user's existing code
  - [ ] Validate snippet security before providing to user
  - [ ] Security validation: Prevent generation of snippets with embedded vulnerabilities

- [ ] **Task 4: Optimize Performance for Sub-2-Second Response (AC: 6)**
  - [ ] Implement caching mechanisms for compiled packages and rule selection
  - [ ] Optimize LLM prompt construction for faster processing
  - [ ] Create performance monitoring and measurement capabilities
  - [ ] Implement timeout handling for slow LLM responses
  - [ ] Security validation: Ensure performance optimizations don't compromise security controls
