...
- OWASP Cheat Sheets and ASVS are our first examples, but the pattern works for any source.
...
1. **Source Documents** (`research/`)
   ...
   - The documents are stored as Markdown.
...
| **Grep** | Explicit (tool) | Minimal | Direct pattern search in rules/corpus | Returns matching lines |
| **CLAUDE.md** | Automatic (patterns) | <1K | Workflow orchestration, security enforcement | Triggers others |
...
1. **Skills are Probabilistic** ⚠️
   ...
   - **CLAUDE.md patterns make a big difference** - in the tests here, skills rarely load without orchestration patterns
